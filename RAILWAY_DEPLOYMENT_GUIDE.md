...
## 7. Registration
Sign in as the `ADMIN_EMAILS` account and open **Admin**. Paste the whole team
into the addresses box, one per line, and generate — each developer gets their
own code, so a seat can be withdrawn from one person later without reissuing
everybody. Send each of them the message the page writes. The codes stay
readable there afterwards, listed against the developer who used them, so
nothing has to be kept in a spreadsheet. Where `REGISTRATION_ACCESS_CODE` is
